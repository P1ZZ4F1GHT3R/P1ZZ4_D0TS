...
resolve_shell_dir() {
	...
	for candidate in \
		"${AMBXST_DIR:-}" \
		"$SCRIPT_DIR" \
		"$HOME/.config/quickshell/Ambxst" \
		"$HOME/.config/quickshell/ambxst" \
		"$HOME/Ambxst" \
		"$HOME/.local/share/ambxst" \
		"$HOME/.local/src/ambxst"; do
...
SHELL_DIR="$(resolve_shell_dir || true)"
...
ensure_config_files() {
	...
	local preset_dir="${SHELL_DIR}/assets/presets/Ambxst Default"
...
Commands:
    ...
    run <action>                      Send an IPC action to the running shell
    matugen <image> [scheme]          Generate Ambxst colors from your rice wallpaper
    theme <image> [scheme]            Generate Ambxst, matugen, and wallust colors
    wallust <image>                   Generate wallust colors from your wallpaper
...
find_ambxst_pid() {
	...
	pid=$(pgrep -f "qs.*${SHELL_DIR:-$SCRIPT_DIR}/shell.qml" 2>/dev/null | head -1)
	if [ -z "$pid" ]; then
		pid=$(pgrep -f "quickshell.*${SHELL_DIR:-$SCRIPT_DIR}/shell.qml" 2>/dev/null | head -1)
	fi
	...
	if [ -z "$pid" ]; then
		pid=$(pgrep -a "qs" 2>/dev/null | grep -F "${SHELL_DIR:-$SCRIPT_DIR}" | awk '{print $1}' | head -1)
	fi
	if [ -z "$pid" ]; then
		pid=$(pgrep -a quickshell 2>/dev/null | grep -F "${SHELL_DIR:-$SCRIPT_DIR}" | awk '{print $1}' | head -1)
	fi
...
matugen)
	shift
	if [ -z "${1:-}" ]; then
		echo "Usage: ambxst matugen <image> [scheme]"
		exit 1
	fi
	"$0" run "matugen $*"
	;;
theme)
	shift
	if [ -z "${1:-}" ]; then
		echo "Usage: ambxst theme <image> [scheme]"
		exit 1
	fi
	"$0" run "theme $*"
	;;
wallust)
	shift
	if [ -z "${1:-}" ]; then
		echo "Usage: ambxst wallust <image>"
		exit 1
	fi
	"$0" run "wallust $*"
	;;
...
brightness)
	...
	if [[ "$ARG2" =~ ^[0-9]+$ ]]; then
		...
		if [ -z "$MONITOR" ]; then
			# Save all monitors
			bash "${SHELL_DIR:-$SCRIPT_DIR}/scripts/brightness_list.sh" >"${BRIGHTNESS_SAVE_FILE}.tmp" 2>/dev/null || {
			...
			CURRENT_LINE=$(bash "${SHELL_DIR:-$SCRIPT_DIR}/scripts/brightness_list.sh" 2>/dev/null | grep "^${MONITOR}:")
	...
	;;
version | -v | --version)
	if [ -n "$SHELL_DIR" ] && [ -f "${SHELL_DIR}/version" ]; then
		echo "Ambxst $(cat "${SHELL_DIR}/version")"
	else
		echo "Ambxst unknown"
	fi
	;;
...
"")
	if [ -z "$SHELL_DIR" ]; then
		echo "Error: Could not find Ambxst shell.qml. Set AMBXST_DIR=/path/to/Ambxst."
		exit 1
	fi

	# Run daemon priority script (backgrounded to not block startup)
	bash "${SHELL_DIR}/scripts/daemon_priority.sh" &
	...
	if [ -n "$NIXGL_BIN" ]; then
		exec "$NIXGL_BIN" "$QS_BIN" -p "${SHELL_DIR}/shell.qml"
	else
		exec "$QS_BIN" -p "${SHELL_DIR}/shell.qml"
	fi
	;;
*)
