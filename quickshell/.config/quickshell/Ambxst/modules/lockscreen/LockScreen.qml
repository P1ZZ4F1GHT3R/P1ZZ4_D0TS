...
WlSessionLockSurface {
    ...
    property string selectedWallpaper: wallpaperPathCollector.text.trim()
    readonly property string lockscreenWallpaper: {
        if (selectedWallpaper === "")
            return "";
        const lower = selectedWallpaper.toLowerCase();
        if (lower.endsWith(".gif")) {
            return Quickshell.env("HOME") + "/.config/waytrogen/gif-frame.jpg";
        }
    ...
    FileView {
        ...
        path: Quickshell.env("HOME") + "/.config/waytrogen/wallpaper.txt"
        watchChanges: true
        onLoaded: root.selectedWallpaper = text().trim()
        onFileChanged: reload()
    }

    Process {
        id: wallpaperFallbackProc
        command: ["sh", "-c", "swww query 2>/dev/null | grep -o 'image: .*' | head -n1 | sed 's/^image: //' || true"]
        running: root.selectedWallpaper === ""

        stdout: StdioCollector {
            id: wallpaperPathCollector
            waitForEnd: true
        }
    }
    ...
    TintedWallpaper {
        ...
        radius: 0
        tintEnabled: false
        source: root.lockscreenWallpaper !== "" ? "file://" + encodeURI(root.lockscreenWallpaper) : ""
        opacity: startAnim && source !== "" ? 1 : 0
        visible: source !== ""
